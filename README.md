...
Our objective was to use the water vole data to empirically test ‘area-isolation’ predictions. To test the ‘area’ prediction we modelled the extinction probability using a Logit Linear model with patch length as a covariate, and to test the ‘isolation’ prediction, we modelled the colonisation probability using a Logit Linear model with connectivity as a covariate. From the description of the metapopulation system, we developed a State-Space formulation of the Dynamic Occupancy model,
 ...
 The dynamic occupancy model showed that connectivity was positively associated with colonisation, supporting the isolation prediction of metapopulation theory. However, patch length had a positive rather than negative association with extinction probability, contrary to the area prediction. The model itself showed good MCMC convergence and good posterior predictive fit, so the unexpected extinction relationship was a substantive finding rather than simply evidence of a poorly fitting model. This suggests that patch length may not fully capture habitat quality or resource availability, or that other ecological processes, such as density dependence or predation, may also be influencing extinction probability not captured by the model.
